...
function generateLocalStyling(inputs: {
  ...
  if (schedule === "exam") {
    recommendationReason = `오늘은 공부와 시험에 온 노력을 쏟아야 하는 '생존' 타이밍입니다. 사용자가 보유 중인 굿즈 중 [${userHas.join(", ") || "의류 기본 아이템"}]을 활용해 보온성과 수납력을 최우선 극대화했습니다. 다소 눅눅할 수 있는 ${weather === "rain" ? "빗길 날씨" : "캠퍼스 분위기"}에 맞게 어두운 톤의 조바심을 막는 편안한 기능성 레이어로 매칭했습니다.`;
  } else if (schedule === "presentation") {
